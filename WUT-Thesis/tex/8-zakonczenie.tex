...
Z~analizy kodu firmware'u Meshtastic\footnote{Meshtastic Firmware. Repozytorium:
\url{https://github.com/meshtastic/firmware}.} oraz opisu API modułów\footnote{Module API.
Dostępne pod adresem: \url{https://meshtastic.org/docs/development/device/module-api/}} wynika,
że częstotliwością raportowania pozycji sterują dwa oddzielne ustawienia: jak często moduł GPS
w~ogóle próbuje złapać nowy namiar, oraz jak często węzeł nadaje posiadaną pozycję do sieci.
...
lokalnie, bez pośrednictwa telefonu - kosztem konieczności utrzymywania własnej gałęzi firmware'u
dla każdego węzła.

Nawet przy takiej zmianie 3~sekundy pozostałyby wynikiem najlepszego przypadku - opóźnienie zależy
też od tego, ile przeskoków pakiet musi pokonać w~sieci mesh i~jak bardzo zajęty jest
współdzielony kanał LoRa, a~nie wyłącznie od częstotliwości próbkowania GPS.
