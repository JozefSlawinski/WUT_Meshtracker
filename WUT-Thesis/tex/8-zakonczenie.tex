...
sztywnego limitu liczby węzłów (21 dla Dogtra, 19 dla Dogtrace). Testy precyzji geofencingu wykazały, 
że wprowadzenie bufora rzędu 6-8~m redukuje liczbę fałszywych alarmów o~99,6\%, co czyni system 
praktycznie użytecznym mimo ograniczeń interwału próbkowania GPS.
...
Gdyby prace nad projektem rozpoczynano od nowa, zasadne byłoby zakupienie różnorodnego zbioru modułów Meshtastic, 
różniących się parametrami anten oraz baterii, aby przetestować różne konfiguracje sprzętowe w roli stacji bazowej 
jeszcze przed ostatecznym wyborem platformy (rozdz.~\ref{sec:modulacja-lora}). Pozwoliłoby to na ocenę zależności zasięgu od typu anteny 
i mocy nadajnika na wcześniejszym etapie projektu oraz, w razie potrzeby, na skorygowanie doboru sprzętu, zanim 
wpłynąłby on na architekturę całego systemu.


\newpage
Na zakończenie pracy przedstawiono często wspominanego w jej treści psa rasy Mały Gończy Gaskoński, 
który stanowił inspirację do powstania niniejszej pracy oraz uczestniczył w testach terenowych.


\begin{figure}[!t]
  \centering
    \includegraphics[width=0.8\linewidth]{reja_meshtracker.jpg}
