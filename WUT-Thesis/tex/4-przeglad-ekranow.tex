...
(Model-View-ViewModel)}~\cite{microsoftMvvm}
...
\begin{figure}[H]
    \centering
    \small
    \begin{tabular}{@{}l@{\hspace{2.5em}}p{8cm}@{}}
        \texttt{ui/}              & \texttt{MainScreen} \\[2pt]
        \quad\texttt{components/} & \texttt{ConnectionStatusBar} \\[2pt]
        \quad\texttt{map/}        & \texttt{MapScreen, MapViewModel} \\[2pt]
        \quad\texttt{nodes/}      & \texttt{NodeDetailScreen, NodeFilterState, NodeItem, NodeListScreen} \\[2pt]
        \quad\texttt{settings/}   & \texttt{SettingsScreen, SettingsViewModel} \\[2pt]
        \quad\texttt{theme/}      & \texttt{Color, Theme, Type} \\[2pt]
        \quad\texttt{zones/}      & \texttt{ZoneBottomSheet, ZoneConfirmDialog, ZoneDetailScreen, ZoneViewModel} \\
    \end{tabular}
...
Węzeł lokalny - czyli połączony z telefonem użytkownika bezpośrednio przez Bluetooth -
...
co ułatwia odnalezienie wybranego urządzenia w większej sieci. Dotknięcie węzła
