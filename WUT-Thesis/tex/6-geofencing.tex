...
\textbf{rzucania promienia} (ang.\ \textit{ray casting})~\cite{geeksforgeeksPip}.
...
Dla każdej krawędzi łączącej wierzchołki $P_i = (x_i, y_i)$
i~$P_j = (x_j, y_j)$ sprawdzany jest warunek przecięcia z promieniem:
...
na prawo od badanego punktu, ponieważ każdy taki promień biegnie w prawo. A każde przecięcie z bokiem strefy neguje bieżącą wartość flagi \texttt{inside}, która na początku wynosi \texttt{false}.
...
przechowywać listy wierzchołków. Rozwiązanie to odrzucono ze względu na brak elastyczności. 
...
Wstępnie rozważano skorzystanie z gotowego geofencing API, udostępnianego przez
Android~\cite{androidGeofencingApi} do monitorowania przekroczeń granic geograficznych. Szybko okazało się jednak, że rozwiązanie to monitoruje
wyłącznie pozycję urządzenia, na którym działa aplikacja, a nie pozycję zewnętrznych
nadajników odbieranych przez sieć Meshtastic,a ponadto obsługuje jedynie strefy
okrągłe, definiowane przez współrzędne środka i promień.
