...
Aplikacja MeshTracker nie komunikuje się bezpośrednio z węzłami sieci, korzysta natomiast
z aplikacji Meshtastic zainstalowanej na tym samym telefonie, która pełni rolę pośrednika. 
Wybrano takie podejście, aby skupić się na warstwie prezentacji i funkcjonalności geofencingu, 
oraz aby nie implementować protokołu komunikacji Meshtastic od nowa.

\paragraph{Mechanizmy IPC.}

Meshtastic udostępnia dwa mechanizmy IPC (ang.\ \textit{Inter-Process Communication}):
...
\begin{itemize}
    \item \textbf{Interfejs AIDL} (ang.\ \textit{Android Interface Definition Language}) -
          ...
          Android wykonuje to za Ciebie za pomocą AIDL~\cite{androidAidlDocs}.}
    ...
    \item \textbf{Broadcasty Androida} - Meshtastic rozsyła komunikaty do wszystkich
        ...
        \footnote{Aplikacje Androida mogą wysyłać i odbierać komunikaty systemowe oraz pochodzące z innych aplikacji, zgodnie ze wzorem projektowym \textit{publish-subscribe}~\cite{androidBroadcastsDocs}.}
\end{itemize}

MeshTracker korzysta z obu mechanizmów jednocześnie: AIDL do inicjalizacji stanu i cyklicznego
odświeżania oraz rozsyłu komunikatów (broadcastów) w celu aktualizacji w czasie rzeczywistym. Gwarantuje to
...
wydaniach. Wymagało to zaimplementowania "łańcucha kandydatów":
...
Odebranie broadcastu \texttt{\seqsplit{ACTION\_MESH\_DISCONNECTED}} \footnote{Broadcasty nie są dostępne w dokumentacji API, jednak można je znaleźć dobrze udokumentowane w repozytorium~\cite{meshtasticAndroidIntentSource}.} przy wciąż aktywnym bindingu
...
\begin{tikzpicture}[
    ...
    state/.style={ellipse, draw=black, thick, fill=blue!7,
        text width=2.6cm, minimum height=1.1cm, align=center},
    stateEnd/.style={ellipse, draw=black, thick, fill=orange!15,
        text width=2.6cm, minimum height=1.1cm, align=center},
    arr/.style={->, >=Stealth, thick},
    lbl/.style={font=\scriptsize, align=center},
]
\node[state]    (connecting) at (0,0)      {\texttt{Connecting(n)}};
\node[state]    (idle)       at (-3.4,-3.4) {\texttt{Disconnected}};
\node[state]    (connected)  at (3.4,-3.4)  {\texttt{Connected}};
\node[stateEnd] (suspended)  at (0,3.0)     {\texttt{Suspended}};

\draw[arr] (idle) -- node[lbl, above left] {\texttt{connect()} lub\\przycisk ,,Ponów''} (connecting);
\draw[arr] (connecting) -- node[lbl, above right] {AIDL: \texttt{getConnectionState()}\\== \texttt{STATE\_CONNECTED}} (connected);
\draw[arr] (connected) to[bend left=20] node[lbl, below] {broadcast\\\texttt{\seqsplit{ACTION\_MESH\_DISCONNECTED}}} (idle);
\draw[arr] (connecting) -- node[lbl, left] {próba nieudana,\\$n \geq n_{\max}$} (suspended);
\draw[arr] (suspended) to[bend left=20] node[lbl, right] {przycisk\\,,Ponów''} (connecting);
\draw[arr] (connecting) to[out=150,in=210,looseness=6] node[lbl, left, xshift=-0.3cm] {próba nieudana,\\$n<n_{\max}$: $n \gets n+1$} (connecting);
\end{tikzpicture}
\caption{Automat stanów procedury ponownego połączenia}
\label{fig:reconnect-fsm}
\end{figure}
...
całkowite (w formacie stopni pomnożonych przez $10^7$)~\cite{meshtasticAndroidNodeSource},
co pozwala na precyzję nawet do około 1 metra bez użycia typów zmiennoprzecinkowych. Natomiast kierunek ruchu (\texttt{groundTrack}) wyrażony jest, z podobnej przyczyny, w stutysięcznych częściach stopnia (pomnożonych przez $10^5$).
...
radia Meshtastic~\cite{meshtasticPositionConfigDocs}.
